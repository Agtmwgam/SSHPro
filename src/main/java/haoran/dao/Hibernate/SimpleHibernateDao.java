 ...
 * 
 * $Id: SimpleHibernateDao.java,v 1.1 2016/03/15 05:48:53 zwm Exp $
 */
...
public class SimpleHibernateDao <T, PK extends Serializable> {
	...
	public void delete(final T entity) {
		Assert.notNull(entity, "entity不能为空");
		  // 非物理删除 标识字段 DelFlag 为1 表示该字段已经被删除掉了 
		 if(entity instanceof IdEntity) {
			 ((IdEntity) entity).setDelFlag(Constants.DELFLAG_UN);
			 getSession().saveOrUpdate(entity);
		 } else {
	 ...
	 * 按属性查找对象列表, 匹配方式为模糊查询.
	 */
	public List<T> likeBy(final String propertyName, final Object value,String orderByProperty,boolean isAsc) {
		Assert.hasText(propertyName, "propertyName不能为空");
		Criterion criterion = Restrictions.ilike(propertyName, (String)value, MatchMode.ANYWHERE);
		Criteria criteria = createCriteria(criterion);
		if (isAsc) {
			criteria.addOrder(Order.asc(orderByProperty));
		} else {
	 ...
	 * 按属性查找对象列表, 匹配方式为模糊.
	 */
	public List<T> likeBy(final String propertyName, final Object value) {
		Assert.hasText(propertyName, "propertyName不能为空");
		Criterion criterion = Restrictions.ilike(propertyName, (String)value, MatchMode.ANYWHERE);
		return find(criterion);
	}
	 ...
	 * @since  1.0.0
	 */
	public boolean checkProperty(String propertyName, String propertyValue, String id){
		if (StringUtils.isEmpty(propertyValue)) {
			return true;
		}
		Object object = findUniqueBy(propertyName, propertyValue);
		Object entity = findUniqueBy("id", id);
		if (object == null) {
			return true;
		} else {
			if (StringUtils.isNotEmpty(id) && object.equals(entity)) {
				return true;
			} else {
				return false;
			}
